...
def _make_effect(effect_type: str, effect_params: dict,
    ...
    if effect_type == "dominate":
        def _fn(card, event, state):
            from engine.card_effects.card_keywords import effect_dominate, _controller_id
            effect_dominate(state, _controller_id(card))
        return _fn

    if effect_type == "mark":
        def _fn(card, event, state):
            from engine.card_effects.card_keywords import effect_mark, _controller_id
            cid = _controller_id(card)
    ...
    if effect_type == "remove_counter":
        ...
        def _fn(card, event, state, _ct=ctype, _amt=amt):
            from engine.card_effects.card_keywords import effect_remove_counter
    ...
    if effect_type == "banish_top_deck":
        ...
        def _fn(card, event, state, _amt=amt, _fu=face_up):
            from engine.card_effects.card_keywords import effect_banish_top_deck, _controller_id
            effect_banish_top_deck(state, _controller_id(card), _amt, face_up=_fu)
        return _fn

    if effect_type == "reload":
        def _fn(card, event, state):
            from engine.card_effects.card_keywords import effect_reload, _controller_id
    ...
    if effect_type == "charge":
        def _fn(card, event, state):
            from engine.card_effects.card_keywords import effect_charge, _controller_id
            cid = _controller_id(card)
