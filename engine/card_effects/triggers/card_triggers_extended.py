...
from engine.card_effects.triggers.triggers import CARD_TRIGGERS, TriggerDef, effect_retrieve_dagger
from engine.card_effects.ability_keywords import (
    _controller_id, _get_controller, _get_opponent_of, _ask_player,
    _remove_from_current_zone,
    create_token, effect_crowd_boos, has_been_booed,
    effect_banish_from_soul, effect_banish_from_hand, effect_move_to_soul,
    effect_reveal_top, effect_look_top,
...
from engine.effect_keywords import (
    ...
    search as _ek_search,
    shuffle as _ek_shuffle,
    amp as _ek_amp,
    charge as _ek_charge,
    mark as _ek_mark,
    negate as _ek_negate,
    reload as _ek_reload,
)
from engine.card_effects.registry import TURN_ATTACK_EFFECTS

# ============================================================
# Compatibility wrappers: old effect_* API -> new effect_keywords API
# ============================================================
...
def effect_draw(state, player_id, n=1):
    _ek_draw(state, player_id, number=n)


def effect_discard(state, player_id, count=1, random_discard=False):
    ...
    for _ in range(count):
        if not player.hand.cards:
            break
        if random_discard:
            card = _random.choice(player.hand.cards)
        else:
...
def effect_banish(state, arg2, arg3=None, face_up=True):
    """Supports two old call patterns:
    - effect_banish(state, player_id, card)  [pid first]
    - effect_banish(state, card, face_up=True)  [card first, no pid]
    """
    if isinstance(arg2, int):
        # effect_banish(state, pid, card)
        pid = arg2
        card = arg3
    else:
    ...
    origin = getattr(card, 'zone', 'hand') or 'hand'
    _ek_banish(state, card, pid, origin)


def effect_banish_top_deck(state, player_id, count=1, face_up=True):
    banished = []
    player = state.players[player_id]
    for _ in range(count):
        ...
        _ek_banish(state, top, player_id, 'deck')
        banished.append(top)
    return banished


def effect_deal_damage(state, player_id, amount):
    player = state.players[player_id]
...
def effect_deal_arcane(state, player_id, amount, source=None):
    player = state.players[player_id]
    target_card = player.hero
    if target_card is None:
        return 0
    src_pid = 3 - player_id
    evt = _ek_deal_damage(state, amount, _DamageType.ARCANE, src_pid,
                          target_card, 'effect', damage_source_card=source)
    return 0 if evt.canceled else amount


def effect_deal_arcane_to_target(state, target, amount, source=None):
    src_pid = _controller_id(target) if hasattr(target, 'controller') else None
    src_pid = (3 - src_pid) if src_pid is not None else 1
    evt = _ek_deal_damage(state, amount, _DamageType.ARCANE, src_pid,
                          target, 'effect', damage_source_card=source)
    return 0 if evt.canceled else amount


def effect_gain_life(state, player_id, n):
    _ek_gain(state, _AssetType.LIFE, n, player_id, target_player_id=player_id)


def effect_lose_life(state, player_id, n):
    player = state.players[player_id]
    if hasattr(player, 'hero') and player.hero is not None:
        player.hero.life = max(0, (player.hero.life or 0) - n)
    else:
        player.life = max(0, (getattr(player, 'life', 0) or 0) - n)


def effect_gain_resources(state, player_id, n):
    _ek_gain(state, _AssetType.RESOURCES, n, player_id, target_player_id=player_id)


def effect_gain_action_point(state, player_id, n=1):
    _ek_gain(state, _AssetType.ACTION_POINTS, n, player_id, target_player_id=player_id)


def effect_destroy(state, player_id, card):
    _ek_destroy(state, card, card)


def effect_opt(state, player_id, n):
    _ek_opt(state, n, player_id)


def effect_intimidate(state, target_player_id):
    source_player_id = 3 - target_player_id
    _ek_intimidate(state, source_player_id, target_player_id)


def effect_put_counter(state, card, counter_type, amount=1):
    _ek_put_counter(state, counter_type, card)


def effect_remove_counter(state, card, counter_type, amount=1):
    _ek_remove_counter(state, counter_type, card)


def effect_search_deck(state, player_id, **kwargs):
    _ek_search(state, **kwargs)


def effect_shuffle(state, player_id):
    _ek_shuffle(state, player_id)


def effect_amp(state, player_id, n):
    _ek_amp(state, n, player_id)


def effect_charge(state, player_id, card):
    _ek_charge(state, card, player_id)


def effect_mark(state, target_player_id):
    _ek_mark(state, target_player_id)


def is_marked(state, player_id):
    return state.players[player_id].class_counters.get("marked", 0) > 0


def effect_negate(state, layer):
    _ek_negate(state, layer)


def effect_reload(state, player_id, card):
    _ek_reload(state, card, player_id)
...
# Ward 4 static ability (ability_keywords has 'Ward' without number — registered manually).
...
def _adrenaline_rush_play(card, event, state):
    ...
    if my_health < opp_health:
        card.effects.append(CardEffect(prop="power", stage=7, substage=5, fn=lambda val: val + 3))
...
def _bet_big_attacking(card, event, state):
    if not _is_this_attacking(card, event, state):
        return
    from engine.card_effects.ability_keywords import add_wager
    cid = _controller_id(card)
...
# -- descendent_gustwave -- migrated to card_effects/json/wtr/descendent_gustwave_{red,yellow,blue}.json
...
def _good_natured_defend(card, event, state):
    ...
    if not state.players[cid].hand.cards:
        card.effects.append(CardEffect(prop="defense", stage=7, substage=5, fn=lambda val: val + 6))
...
# -- retrace_the_past_blue -- migrated to card_effects/json/wtr/retrace_the_past_blue.json
...
def _right_behind_you_defend(card, event, state):
    ...
    if state.combat and len(state.combat.defending_cards) >= 2:
        card.effects.append(CardEffect(prop="defense", stage=7, substage=5, fn=lambda val: val + 1))
...
# -- sand_sketched_plan_blue -- migrated to card_effects/json/wtr/sand_sketched_plan_blue.json
...
# -- sharpen_steel_red -- migrated to card_effects/json/wtr/sharpen_steel_red.json
...
# -- sigil_of_solace_red -- migrated to card_effects/json/wtr/sigil_of_solace_{red,yellow,blue}.json
...
# -- snatch -- migrated to card_effects/json/wtr/snatch_{red,yellow,blue}.json
...
_register("steel_street_enforcement", [
    TriggerDef(event_type="defend",
               effect_fn=lambda c, e, s: c.effects.append(CardEffect(prop="defense", stage=7, substage=5, fn=lambda val: val + 1))
               if _is_this_defending(c, e, s) else None),
])

# -- steelblade_supremacy_red -- migrated to card_effects/json/wtr/steelblade_supremacy_red.json
...
_register("turning_point", [
    TriggerDef(event_type="defend",
               effect_fn=lambda c, e, s: c.effects.append(CardEffect(prop="defense", stage=7, substage=5, fn=lambda val: val + 2))
               if _is_this_defending(c, e, s) else None),
])
...
# -- whelming_gustwave -- migrated to card_effects/json/wtr/whelming_gustwave_{red,blue}.json
...
# -- ironsong_response -- migrated to card_effects/json/wtr/ironsong_response_red.json
...
# -- nip_at_the_heels_blue -- migrated to card_effects/json/wtr/nip_at_the_heels_blue.json
# -- overpower_{red,yellow,blue} -- migrated to card_effects/json/wtr/overpower_{red,yellow,blue}.json
...
# -- singing_steelblade_yellow -- migrated to card_effects/json/wtr/singing_steelblade_yellow.json
...
def _scepter_of_pain_activate(card, event, state):
    cid = _controller_id(card)
    target_id = 3 - cid
    dmg = effect_deal_arcane(state, target_id, 1, source=card)
    dealt = dmg or 0
...
def _a_drop_in_the_ocean_play(card, event, state):
    from engine.card_effects.ability_keywords import _ask_player, _remove_from_current_zone, effect_transcend
    cid = _controller_id(card)
    ...
    chosen.effects.append(CardEffect(prop="power", stage=7, substage=6, fn=lambda val: val - 1))
