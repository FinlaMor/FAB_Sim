...
#   Per-card handlers for unique or complex statics that aren't keyword-based.
# ---------------------------------------------------------------------------
...
# keyword_prefix -> fn(n: int, state, card) -> None
...
# slug -> list of (event_name, fn(event, state, card))
CARD_STATIC_ABILITIES: dict = {}


# ── Outmuscle (all colors) ────────────────────────────────────────────────
# "Go Again — while Outmuscle is not blocked by a card with 6 or more power."
# (Yellow/Blue thresholds differ — simplified here to card's own attack power.)
_OUTMUSCLE_THRESHOLDS = {
    'outmuscle_red': 6,
    'outmuscle_yellow': 5,
    'outmuscle_blue': 4,
}

def _outmuscle_static(event, state, card) -> None:
    if not state.combat:
        return
    threshold = _OUTMUSCLE_THRESHOLDS.get(card.slug, 6)
    defending_max = max(
        (c.power or 0) for c in state.combat.defending_cards
    ) if state.combat.defending_cards else 0
    if defending_max >= threshold:
        state.combat.revoke_keyword('Go Again')
    else:
        state.combat.grant_keyword('Go Again')

for _slug in _OUTMUSCLE_THRESHOLDS:
    CARD_STATIC_ABILITIES[_slug] = [('recalculate_attack_power', _outmuscle_static)]


# ── Merciful Retribution ──────────────────────────────────────────────────
# "Whenever an aura you control is destroyed, deal 1 arcane damage to target hero."
def _merciful_retribution_static(event, state, card) -> None:
    from engine.engine import _apply_arcane_damage
    # Find which player owns this Merciful Retribution
    owner_id = None
    for pid, player in state.players.items():
        if card in player.auras.cards or card in player.items.cards or card in player.permanents.cards:
            owner_id = pid
            break
    if owner_id is None:
        return
    # Only fire when one of the owner's auras was the destroyed one
    destroyed_card = event.data.get('card')
    destroyed_owner = event.data.get('player_id')
    if destroyed_owner != owner_id:
        return
    _apply_arcane_damage(state, 3 - owner_id, 1)

CARD_STATIC_ABILITIES['merciful_retribution'] = [('aura_destroyed', _merciful_retribution_static)]
...
def _aether_ironweave_condition(player, slot_name, equip_card, state):
    """Aether Ironweave: only activatable if you have played both an attack action
    and a non-attack action card this turn."""
    played = getattr(player, 'cards_played_this_turn', [])
    has_attack_action = any(
        'Action' in (c.types or []) and 'Attack' in (c.subtypes or [])
        for c in played
    )
    has_non_attack_action = any(
        'Action' in (c.types or []) and 'Attack' not in (c.subtypes or [])
        for c in played
    )
    return has_attack_action and has_non_attack_action

EQUIPMENT_ACTIVATION_CONDITIONS["aether_ironweave"] = _aether_ironweave_condition
...
def _marlynn_create_harpoon(action, player, state):
    ...
    from engine.card_effects.ability_keywords import create_token_card
...
def _oscilio_discard_instant_draw(action, player, state):
    ...
    draw(state, draw_player=player.player_id, number=1)
...
def _achilles_accelerator_condition(player, slot_name, equip_card, state=None) -> bool:
    ...
    return "boosted_this_turn" in player.current_turn_effects

def _storm_striders_effect(action, player, state):
    """Effect: next Wizard non-attack action card this turn costs {r} less."""
    state.continuous_effect_manager.add_cost_modifier(
        kind="decrease",
        amount=1,
        owner_player_id=player.player_id,
        effect_id="storm_striders_cost_reduction",
        condition_fn=lambda card, act, s: (
            "wizard" in [t.lower() for t in (card.types or [])]
            and not getattr(card, 'is_attack', False)
        ),
        consume_on_apply=True,
        source_slug="storm_striders",
    )
...
def _adaptive_alpha_mold_effect(action, player, state):
    """Shared effect for Adaptive Alpha Mold / Dissolver / Plating.
    Move the card from its current slot to another empty equipment slot
    (head/chest/arms/legs). If multiple empty slots exist, ask the player to
    choose. If no empty slot exists, resolves with no movement."""
    card = action.card
    # Find current slot
    current_slot = None
    ...
    if current_slot is None:
        return  # shouldn't happen; card must be in a slot to be activated

    # Find empty equipment slots (other than current)
    empty_slots = [
        s for s in ("head", "chest", "arms", "legs")
        if s != current_slot and not player.zone_by_name(s).cards
    ]
    if not empty_slots:
        return  # no empty destination — ability resolves with no movement
    ...
    # Move card from current slot to destination slot
    src_zone = player.zone_by_name(current_slot)
    dst_zone = player.zone_by_name(dest_slot)
    src_zone.remove(card)
    dst_zone.add(card, is_public=True)  # Zone.add sets card.zone = dest_slot
...
def _volzar_effect(action, player, state):
    ...
    if lightning_count > 0:
        amp(state, amount=lightning_count, player_id=player.player_id)
...
def _quiver_effect(action, player, state):
    ...
    from engine.card_effects.ability_keywords import _ask_player
    from engine.effect_keywords import shuffle
    ...
    shuffle(state, target_player_id=player.player_id)
...
def _aether_conduit_effect(action, player, state):
    ...
    if target_card is not None and hasattr(target_card, 'owner'):
        ...
        target_id = 3 - player.player_id  # fallback: opponent
    target_hero = state.players[target_id].hero
    deal_damage(state, amount=2, damage_type=DamageType.ARCANE,
                source_player_id=player.player_id, damage_target=target_hero,
                damage_source='card', damage_source_card=action.card)
...
EQUIPMENT_ACTIVATION_EFFECTS = {
    "adaptive_alpha_mold": _adaptive_alpha_mold_effect,
    "adaptive_dissolver": _adaptive_alpha_mold_effect,
    "adaptive_plating": _adaptive_alpha_mold_effect,
    ...
    "aether_conduit": _aether_conduit_effect,
...
def _aether_ironweave_effect(action, player, state):
    """Gain {r}{r} and Go Again. Destroy-self cost handled by generic engine fallback."""
    player.resources += 2
    player.action_points += 1  # Go Again (equipment activations bypass stack, no auto-GA)

EQUIPMENT_ACTIVATION_EFFECTS["aether_ironweave"] = _aether_ironweave_effect
...
def _nimblism_next_attack_plus1_apply(attack_card, player, state):
    attack_card.effects.append(CardEffect(prop="power", stage=7, substage=5, fn=lambda val: val + 1))
...
def _hammerhead_next_arrow_apply(attack_card, player, state):
    ...
    attack_card.effects.append(CardEffect(prop="power", stage=7, substage=5, fn=lambda val: val + 4))
...
def _cheating_scoundrel_next_attack_apply(attack_card, player, state):
    ...
    attack_card.effects.append(CardEffect(prop="power", stage=7, substage=5, fn=lambda val: val + 3))
...
def _under_the_trap_door_discard_effect(action, player, state):
    ...
    instead banish it."""
    from engine.card_effects.ability_keywords import _ask_player, effect_banish
...
def _next_attack_plus2_apply(attack_card, player, state):
    attack_card.effects.append(CardEffect(prop="power", stage=7, substage=5, fn=lambda val: val + 2))
...
def _item_gain_2h_go_again(action, player, state):
    ...
    from engine.effect_keywords import gain, AssetType
    gain(state, AssetType.LIFE, 2, player.player_id, target_player_id=player.player_id)
    player.action_points += 1

def _item_opt2(action, player, state):
    """Opt 2. (clarity_potion_blue, opal_amulet_blue)"""
    from engine.card_effects.ability_keywords import _ask_player
    from engine.effect_keywords import opt
    pid = player.player_id
    def _selector(cards):
        top, bottom = [], []
        for card in cards:
            choice = _ask_player(state, pid, ['top', 'bottom'],
                                 context=f"Opt: {card.slug} top or bottom?")
            (top if choice == 'top' else bottom).append(card)
        return top, bottom
    opt(state, n=2, target_player_id=pid, selector=_selector, source_player_id=pid)
...
def _potion_of_luck_effect(action, player, state):
    ...
    from engine.effect_keywords import draw, shuffle
    ...
    for c in list(player.arsenal.cards):
        ...
        player.deck.add_bottom(c)
    shuffle(state, target_player_id=player.player_id)
    draw(state, draw_player=player.player_id, number=total)
...
def _amulet_of_echoes_effect(action, player, state):
    ...
    import random
    from engine.effect_keywords import discard as _discard
    opp = state.players[3 - player.player_id]
    name_counts: dict = {}
    ...
    if any(v >= 2 for v in name_counts.values()):
        for _ in range(2):
            if not opp.hand.cards:
                break
            card = random.choice(opp.hand.cards)
...
def _seeker_kunai_effect(action, player, state):
    ...
    if state.combat and state.combat.attack_card:
        ...
        if "assassin" in types_lower:
            ...
            state.combat.attack_card.effects.append(CardEffect(prop="power", stage=7, substage=5, fn=lambda val: val + 1))
...
def _rhinar_reckless_discard_trigger(player, event, state):
    ...
    if power >= 6:
        ...
        intimidate(state, source_player_id=player.player_id, target_player_id=opp_id)
    ...
    if count == 1:  # This is the second attack (count was 1 before append)
        if state.combat and state.combat.attack_card:
            ...
            if ac.controller == player.player_id:
                ...
                ac.effects.append(CardEffect(prop="power", stage=7, substage=5, fn=lambda val: val + 1))
...
def _kassai_draw_trigger(player, event, state):
    ...
    if not any(e.effect_id == "kassai_sword_cost_reduction" for e in mgr._effects if e.prop == 'cost'):
        mgr.add_cost_modifier(
            kind="decrease",
            amount=1,
            owner_player_id=player.player_id,
            effect_id="kassai_sword_cost_reduction",
            condition_fn=lambda card, act, s: (
                act.type.value == "attack_weapon"
                and any("sword" in t.lower() for t in (card.types or []))
            ),
            consume_on_apply=False,  # persists until end of turn
            source_slug="kassai",
        )
...
def _vynnset_start_of_turn_trigger(player, event, state):
    ...
    if not player.hand.cards:
        return
    from engine.card_effects.ability_keywords import _ask_player, effect_banish, create_token
    pick = _ask_player(state, player.player_id, [c.slug for c in player.hand.cards],
...
def _maxx_hyper_driver_crank_trigger(player, event, state):
    ...
    if "hyper" not in card.slug or "driver" not in card.slug:
        return
    from engine.card_effects.ability_keywords import crank
    crank(card, state)
...
def _hala_activate(action, player, state):
    ...
    target.effects.append(CardEffect(prop="power", stage=7, substage=5, fn=lambda val: val + 1))
...
def _oscilio_forked_pay_cost(player, state):
    ...
    if player.hand.cards:
        ...
        was_instant = card.is_instant
        slug = card.slug
        _discard(state, discard_target=card, discard_source=None, origin='hand')
        if was_instant:
            state._oscilio_forked_discarded_instant = slug
        else:
...
def _arakni_marionette_attack_trigger(player, event, state):
    ...
    combat.attack_card.effects.append(CardEffect(prop="power", stage=7, substage=5, fn=lambda val: val + 1))
...
def _gravy_effect(action, player, state):
    ...
    import random
    from engine.effect_keywords import draw, discard as _discard
    draw(state, draw_player=player.player_id, number=1)
    if player.hand.cards:
        card = random.choice(player.hand.cards)
        _discard(state, discard_target=card, discard_source=None, origin='hand')
...
def _lyath_effect(action, player, state):
    ...
    from engine.effect_keywords import boo
    boo(state, target_player_id=player.player_id)
...
def _prism_awaken_effect(action, player, state):
    ...
    from engine.card_effects.ability_keywords import _ask_player, effect_awaken, FIGMENT_TO_ANGEL
...
def _zyggy_pay_cost(player, state):
    ...
    flow = flows[0]
    from engine.engine import destroy_aura
    destroy_aura(state, player.player_id, flow)
    return True
...
def _arakni_huntsman_play_trigger(player, event, state):
    ...
    if not opp.deck.cards:
        return
    from engine.card_effects.ability_keywords import _ask_player, effect_look_top
...
def _fang_hit_trigger(player, event, state):
    ...
    mgr = state.continuous_effect_manager
    if fealty_count >= 3:
        if not any(e.effect_id == "fang_dagger_cost_0" for e in mgr._effects if e.prop == 'cost'):
            mgr.add_cost_modifier(
                kind="set",
                amount=0,
                owner_player_id=player.player_id,
                effect_id="fang_dagger_cost_0",
                condition_fn=lambda card, act, s: act.type.value == "attack_weapon",
                consume_on_apply=False,  # re-evaluated each time; removed below if Fealty drops
                source_slug="fang_dagger",
            )
    else:
        # Clear if Fealty dropped below 3
        mgr.remove_cost_modifier_by_id("fang_dagger_cost_0")
